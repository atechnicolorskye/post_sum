...
def penalty_objective(Z_0, Z_1, Z_2, psi, theta):
    ...
    print(theta * sum(map(l1_od_norm, Z_0)), (1 - theta) * sum(map(psi, Z_2 - Z_1)))
    return theta * sum(map(l1_od_norm, Z_0)) + (1 - theta) * sum(map(psi, Z_2 - Z_1))


tic = time.perf_counter()
tgl = TimeGraphicalLasso(alpha=1., beta=1., theta=0.5, mode='admm', rho=1, tol=1e-4,
...
for _weights in [[None], ['lin', 2e2], ['exp', 2e2]]:
    # Set parameters
    max_iter = 10
    loss = 'LL'
    c_level = 0.2
    ...
    # plt.tight_layout()
    # plt.savefig('syn_' + str(weights[0]).lower() + '_diff_like.pdf')
    ...
    # plt.tight_layout()
    # plt.savefig('syn_' + str(weights[0]).lower() + '_diff_support.pdf')
    ...
    # plt.tight_layout()
    # plt.savefig('syn_' + str(weights[0]).lower() + '_diff_fro.pdf')